...
/**
 * @file KFitDecayBuilder.cxx
 * @brief Implementation of candidate combinatorics and best-fit selection.
 */

KFitDecayBuilder::KFitDecayBuilder(TString task, std::vector<int> pids,
                                   TLorentzVector lv, double mass)
    : fTask(task), fPids(pids), fVerbose(0)

{
  if (fVerbose > 0) {
    std::cout << "--------------- KFitDecayBuilder() -----------------"
              << std::endl;
  }
...
void KFitDecayBuilder::buildDecay() {
  if (fVerbose > 0) {
    std::cout
        << "--------------- KFitDecayBuilder::buildDecay() -----------------"
        << std::endl;
  }
  if (fVerbose > 1) {
    std::cout << "Combi counter: " << fCombiCounter
              << " total Combos: " << fTotalCombis << std::endl;
    std::cout << "Task is " << fTask << std::endl;
  }

  // Select best combination according to fit probability.
  fBestProb = 0;
  fBestChi2 = 1e6;
...
Bool_t KFitDecayBuilder::doFit() {
  if (fVerbose > 0) {
    std::cout << "--------------- KFitDecayBuilder::doFit() -----------------"
              << std::endl;
  }
...
void KFitDecayBuilder::fillFitCands() {
  ...
  for (size_t i = 0; i < fPids.size(); i++) {
    ...
    if (fVerbose > 1)
      std::cout << "particle counter " << i << " is " << particleCounter[i]
                << " now" << std::endl;
  ...
  int a = fPids.size() - 1;
  if (fVerbose > 1)
    std::cout << "particle counter " << a << " is " << particleCounter[a]
              << " now" << std::endl;
  while (particleCounter[a] == (((int)fCands[a].size()) - 1)) {
    ...
    if (a < 0) {
      if (!(fCombiCounter == fTotalCombis - 1))
        if (fVerbose > 1)
          std::cout << "counted wrong: " << fCombiCounter
                    << " != " << fTotalCombis << std::endl;
      break;
  ...
  if (doubleParticle && (fCombiCounter < fTotalCombis))
    fillFitCands(); // If some particle has been filled more than once into
                    // fFitCands, repeat with the next combination.
}

void KFitDecayBuilder::checkDoubleParticle(size_t i) {
  if (fVerbose > 0) {
    std::cout << "--------------- KFitDecayBuilder::checkDoubleParticle() "
                 "-----------------"
              << std::endl;
  }
