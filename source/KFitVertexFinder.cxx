...
/**
 * @file KFitVertexFinder.cxx
 * @brief Implementation of least-squares vertex finding from track lines.
 */

KFitVertexFinder::KFitVertexFinder(std::vector<KinFitParticle> &cands)
    : fVerbose(0), fCands(cands) {
  ...
  for (int i_cand = 0; i_cand < fCands.size(); i_cand++) {
    ...
    addLinesToVertex(fBase, fDir,
                     1.0); // Add one track line to the vertex system.
  }

  findVertex(); // Solve the linear system and store the fitted vertex.
}

void KFitVertexFinder::addLinesToVertex(const TVector3 &r,
                                        const TVector3 &alpha, const double w) {

  if (fVerbose > 0) {
    std::cout << " ----------- KFitVertexFinder::addLinesToVertex() -----------"
              << std::endl;
    std::cout << "" << std::endl;
  }
...
void KFitVertexFinder::findVertex() {
  ...
  if (fVerbose > 0) {
    std::cout << " ----------- KFitVertexFinder::findVertex() -----------"
              << std::endl;
    std::cout << "" << std::endl;
  }
  ...
  det = fSys(0, 0) * fSys(1, 1) * fSys(2, 2) +
        fSys(0, 1) * fSys(1, 2) * fSys(0, 2) +
        fSys(0, 1) * fSys(1, 2) * fSys(0, 2) -
        fSys(0, 2) * fSys(1, 1) * fSys(0, 2) -
        fSys(0, 1) * fSys(0, 1) * fSys(2, 2) -
        fSys(1, 2) * fSys(1, 2) * fSys(0, 0);
